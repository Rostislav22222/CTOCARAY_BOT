...
def handle_start(message):
    ...
    if str(user_id) == admin_chat_id:
        login_button = types.KeyboardButton("Увійти в адмін-панель")
        check_orders_button = types.KeyboardButton("Перевірити замовлення")
        markup.row(login_button)
        markup.row(check_orders_button)
    ...
    markup.row(appointment_button)  # Додана кнопка для запису на технічне обслуговування
